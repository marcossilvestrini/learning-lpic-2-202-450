...
echo -e $LINE >>$FILE_TEST
echo -e "Check file /etc/named.conf...\n" >>$FILE_TEST
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@192.168.0.141 -l vagrant sudo cat /etc/bind/named.conf.local | grep -ws "lpic2.com.br" -A 14 >>$FILE_TEST
echo -e $LINE >>$FILE_TEST
...
echo -e $LINE >>$FILE_TEST
echo -e "Check file /etc/named.conf...\n" >>$FILE_TEST
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@192.168.0.142 -l vagrant sudo cat /etc/bind/named.conf.local | grep -ws "lpic2.com.br" -A 14 >>$FILE_TEST
echo -e $LINE >>$FILE_TEST
